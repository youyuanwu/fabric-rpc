...
// This is needed because we do not want fabric_rpc.lib to have dependency on protobuf.lib
// User should be able to provide the protobuf.lib and link with the generated code.
// Otherwise we will have 2 copies (potentially 2 versions) of protobuf.lib in the final executable.
class IFabricRPCHeaderProtoConverter{
public:
  virtual bool SerializeRequestHeader(const FabricRPCRequestHeader * request, std::string * data) = 0;
...
public:
  // returns true if ok
  bool SerializeRequestHeader(const FabricRPCRequestHeader * request, std::string * data) override {
  ...
  bool SerializeReplyHeader(const FabricRPCReplyHeader * reply, std::string * data) override {
  ...
  bool DeserializeRequestHeader(const std::string * data, FabricRPCRequestHeader * request) override {
    assert(request != nullptr);
    assert(data != nullptr);
    RequestProto header;
  ...
  bool DeserializeReplyHeader(const std::string * data, FabricRPCReplyHeader * reply) override {
    assert(reply != nullptr);
    assert(data != nullptr);
    ReplyProto header;
    if(!header.ParseFromString(*data)){
